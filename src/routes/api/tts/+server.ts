...
const chunk = (text: string, len = 180) => { //safe mode 180char
  let curr = len,
    prev = 0,
    output = [];
...
const getAudio = async (text: string) => {
  ...
  const buffer = await Promise.all(
    textArray.map(async (str) => {
      const body = await fetch(`https://texttospeech.responsivevoice.org/v1/text:synthesize?text=${encodeURIComponent(str)}&lang=id&engine=g3&name=&pitch=0.5&rate=0.5&volume=1&key=kvfbSITh&gender=male`);
